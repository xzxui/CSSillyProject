...
def SaveMarkingResultToExcel(save_path:str, syllabus_code:str, component_number:str, marking_report:list, strengths:str, weaknesses:str, score:int, grade:str):
    ...
    """
    Marking_report_columns = ['Question Number', 'Maximum Mark Possible', 'Marks Received'] #Columns for marking report array
    ...
    df_result = pd.DataFrame({'Score':[score], 'Grade':[grade]})    #Saving input to seperate dataframes

    df_sheet=pd.concat([df_paper, df_report,df_result, df_feekback], axis=1) #Combining dataframes into one
