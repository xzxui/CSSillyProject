...
def ProduceMarkingReport(student_work_b64imgs, mark_scheme_b64imgs):
    ...
    marking_report = ModuleLLMQuery.LLMQuery(
        [
            {"role": "system", "content": "You are a highly experienced senior A-Level examiner who is marking the answers of a candidate. A detailed mark scheme is provided with instructions you will follow consistently and perfectly when grading the candidate's work. The user will provide you the mark scheme and the completed question paper."},
            {"role": "user", "content": [
                {
                    "type": "text",
                    "text": "Here are the completed question paper and the mark scheme."
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/{configs.img_extension};base64,{concat_b64imgs(student_work_b64imgs)}"
                    }
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/{configs.img_extension};base64,{concat_b64imgs(mark_scheme_b64imgs)}"
                    }
                },
            ]}
        ],
...
if __name__ == "__main__":
    ...
    b64_imgs = ModulePDF2b64s.PDF2b64s("test_folder/original.pdf")
    with open("test_folder/merged.png", "wb") as f:
        f.write(base64.b64decode(concat_b64imgs(b64_imgs)))
